...
public interface SongRepository extends JpaRepository<Song, Integer> {
    ...
    @Query("SELECT s, COUNT(f.id) as likeCount FROM Song s LEFT JOIN Favorite f ON s.id = f.song.id " +
           ...
           "(:endDate IS NULL OR s.createdAt <= :endDate) " +
           "GROUP BY s.id, s.title, s.prompt, s.audioUrl, s.status, s.isPublic, s.lyrics, s.modelVer, s.isRemix, s.parentId, s.coverUrl, s.listenCount, s.createdAt, s.user, s.generationProvider, s.providerTaskId, s.providerStatus, s.generationDurationSeconds, s.vocalMode, s.vocalLanguage, s.vocalGender " +

...
public interface OrderRepository extends JpaRepository<Order, Integer>, JpaSpecificationExecutor<Order> {
	...
	@Query("SELECT o FROM Order o WHERE (:from IS NULL OR o.createdAt >= :from) AND (:to IS NULL OR o.createdAt <= :to) AND (:status IS NULL OR o.status = :status)")
	Page<Order> findFiltered(@Param("from") Date from, @Param("to") Date to, @Param("status") String status, Pageable pageable);
}
